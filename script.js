...
//clamp is set from range lie on console
    xprev = dets.clientX;
    yprev = dets.clientY;
    ...
    timeout = setTimeout(function () {
      document.querySelector(
        "#minicircle"
      ).style.transform = `translate(${dets.clientX}px, ${dets.clientY}px) scale(1, 1)`; //x and y scale from 0 to 1 show movement
    }, 100);
  });
}
...
document.querySelectorAll(".elem").forEach(function (elem) {
  ...
  elem.addEventListener("mousemove", function (dets) {
    ...
    gsap.to(elem.querySelector("img"), {
      ...
      //the image ratio lies from the given range not from 180 to 360.
    });
  });
});
